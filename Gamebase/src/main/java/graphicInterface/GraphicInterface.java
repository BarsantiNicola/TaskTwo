...
public class GraphicInterface {
		...
		for( int i=0; i < imagesURLList.size(); i++ ) {
			...
			if( icon == null ) {
				...
				try {
					...
					try {
						scaledImage = ImageIO.read(new File("/resources/defaultGamePicture.png")).getScaledInstance(310, 200, Image.SCALE_SMOOTH);
				...
				scaledImage = icon.getImage().getScaledInstance(310, 200, Image.SCALE_SMOOTH);
			}
			
			imagesListModel.addElement(scaledImage);
		}
	}
		...
		if( mostViewedStatus.statusCode == StatusCode.OK ) {
			...
			if( icon == null ) {
				...
				try {
					...
					icon = new ImageIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/defaultGamePicture.png")).getImage().getScaledInstance(238, 155, Image.SCALE_FAST));
				}
				...
				icon = new ImageIcon(icon.getImage().getScaledInstance(238, 155, Image.SCALE_FAST));
	...
	private void initializeGamePage( int id ) {
		...
		try {
			...
			background_image_icon = new ImageIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/defaultGamePicture.png")).getImage().getScaledInstance(342, 188, Image.SCALE_SMOOTH));
		}
	...
	private void initializeSearchGamePage() {
		...
		StatusObject<List<GraphGame>> featuredGamesStatus = logicHandler.getFeaturedGamesList(searchedGamesPerPage);
	...
	private void initialize() {
		...
		featuredButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				StatusObject<List<GraphGame>> featuredStatusObject  = logicHandler.getFeaturedGamesList(searchedGamesPerPage);
		...
		searchGamesVerticalScrollBarListener =  new AdjustmentListener() {
		      public void adjustmentValueChanged(AdjustmentEvent e) {
			      ...
			      searchGamesVerticalScrollBar.removeAdjustmentListener(searchGamesVerticalScrollBarListener);
			      ...
			      SwingWorker<List<BufferedGame>,Void> loadGamesPageSwingWorker = new SwingWorker<List<BufferedGame>,Void>() {
						...
						protected void done() {
						    ...
						    searchGamesVerticalScrollBar.setValue(1);
		...
		vote1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( logicHandler.vote(currentGame.getId().toString(), 1)) {
		...
		vote2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( logicHandler.vote(currentGame.getId().toString(), 2) ) {
		...
		vote3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( logicHandler.vote(currentGame.getId().toString(), 3) ) {                   
		...
		vote4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( logicHandler.vote(currentGame.getId().toString(), 4) ) {              
		...
		vote5.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( logicHandler.vote(currentGame.getId().toString(), 5) ) {               
		...
		assignedVoteLabel.setBounds(774, 277, 113, 22);
